...
- Add `ServerSideApply=true` to syncOptions for large CRDs (see `17-cloud-native-pg.yaml`, `18-azure-service-operator.yaml`, `20-k6-operator.yaml`)
- Skip `ServerSideApply` for plain workload addons with no CRDs (see `21-wiremock.yaml`)
...
- 19+: Workload operators and testing services (k6-operator, wiremock)
